...
Checkout git sources into /tmp/llvm-build/src. Used inside a docker container.
...
LLVM_BUILD_DIR=/tmp/llvm-build

# Get the sources from git.
echo "Checking out sources from git"
mkdir -p "$LLVM_BUILD_DIR/src"
CHECKOUT_DIR="$LLVM_BUILD_DIR/src"
...
if [ -f "$CHECKSUMS_FILE" ]; then
  ...
  python "$(dirname $0)/llvm_checksum/llvm_checksum.py" -c "$CHECKSUMS_FILE" \
    --partial --multi_dir "$LLVM_BUILD_DIR/src"
else
